...
if [ "$OS" = "Darwin" ]; then
    ...
    echo -e "${YELLOW}Instalando dependencias del sistema (portaudio, ffmpeg, tmux)...${NC}"
    ...
    if command -v apt-get &> /dev/null; then
        echo -e "${YELLOW}Instalando dependencias del sistema (portaudio19-dev, ffmpeg, python3-venv, tmux)...${NC}"
        sudo apt-get update
        sudo apt-get install -y portaudio19-dev ffmpeg python3-venv python3-dev tmux
    elif command -v pacman &> /dev/null; then
        echo -e "${YELLOW}Instalando dependencias del sistema (portaudio, ffmpeg, tmux)...${NC}"
        sudo pacman -Sy --noconfirm portaudio ffmpeg python-virtualenv tmux
    elif command -v dnf &> /dev/null; then
        echo -e "${YELLOW}Instalando dependencias del sistema (portaudio-devel, ffmpeg, tmux)...${NC}"
        sudo dnf install -y portaudio-devel ffmpeg python3-virtualenv tmux
    else
